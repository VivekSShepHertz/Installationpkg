...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			sudo cp -arf /home/azureuser/Installationpkg/sparkha1/app42RDS /.
			...
			if [ $? -eq 0 ]; then
				echo "SparkHA1 Configured Successfully"
			else
				echo "SparkHA1 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "SparkHA1 Config Configuration Failed"
			exit 1
		fi
	else
		echo "SparkHA1 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "SparkHA1 Repo Setup Failed"
	exit 1
fi

else
        echo "SparkHA1 CentOS Repo Setup Failed"
        exit 1
fi
