...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			sudo cp -arf /home/azureuser/Installationpkg/sparkproxyha2/app42RDS /.
			...
			if [ $? -eq 0 ]; then
				echo "SparkProxyHA2 Configured Successfully"
			else
				echo "SparkProxyHA2 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "SparkProxyHA2 Config Configuration Failed"
			exit 1
		fi
	else
		echo "SparkProxyHA2 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "SparkProxyHA2 Repo Setup Failed"
	exit 1
fi

else
        echo "SparkProxyHA2 CentOS Repo Setup Failed"
        exit 1
fi
