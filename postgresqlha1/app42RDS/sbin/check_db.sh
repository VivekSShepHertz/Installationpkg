...
if [ $? -eq 0 ]; then
        mail -s "$setup_name : PostgreSQL Service Running Successfully : PostgreSQLHA 1" $Email < /var/log/postgresql/postgresql-9.6-main.log
else
        mail -s "$setup_name : PostgreSQL Service Starting Failed : PostgreSQLHA 1" $Email < /var/log/postgresql/postgresql-9.6-main.log
fi
else
