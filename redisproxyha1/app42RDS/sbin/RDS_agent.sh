...
get.redis.info)
			...
			load=`uptime |rev|cut -d ',' -f3|rev|cut -d ':' -f2|tr -d ' '`
                       ...
                       # conn=`echo $conn_stat|grep "Threads_connected"|awk '{print $8}'`
                        conn=`echo "INFO" | redis-cli | /bin/grep connected_clients | awk '{print $1}' | cut -d ':' -f2`
                       ...
                       #max_conn=`echo $max_conn_stat|grep "max_connections"|awk '{print $4}'`
                        max_conn=`cat /etc/redis/redis.conf | grep maxclients | awk '{print $2}'`
                        if [ -z "$conn_stat" ]; then
			...
			rm -rf /tmp/err /tmp/err1
                        ...
                        echo '{"success":"false","code":3001, "message":"Current Redis VM Info Could Not Be Fetch, Due To Postgres Not Running"}'
                fi
        ;;
